...
namespace Engine.ProcessCore
{
    public class Module : IDisposable
    {
        ...
        private readonly Logger log = new Logger(LoggerType.Console_File, "ProcessCore.Module");
        ...
        public Module(string dllToLocate)
        {
            ...
            if (File.Exists(dllToLocate))
            {
                ...
                {
                    log.Log(LogType.Error, "Failed to GetModuleHandle from {0}: {1}", Path.GetFileName(dllToLocate),
        ...
        private void LoadInternal(IntPtr newModule)
        {
            if (newModule != IntPtr.Zero)
            {
                ...
                var res = WinAPI.GetModuleFileName(hModule, sb, sb.Capacity);
                if (res != 0)
                {
                    DllName = sb.ToString();
                }
                ...
                {
                    DllName = "!Unresolved!";
                    log.Log(LogType.Failure, "Failed to aquire the file name of the module: {0}",
                        Marshal.GetLastWin32Error().ToString("X"));
                }
            }
            else
