...
namespace Injectors.RtlCreateUserThread
{
    ...
    {
        private readonly Logger log = new Logger(LoggerType.Console_File, "Injector.RCUT");

        public string SelfFileName => Path.GetFileName(Assembly.GetExecutingAssembly().Location);

        public string UniqueId => "Injectors.RCUT-" + QuickExt.GetHash(
                                      Encoding.UTF8.GetBytes(UniqueName +
                                                             Marshal.GetTypeLibGuidForAssembly(
                                                                 Assembly.GetExecutingAssembly())), HashType.MD5);

        public string UniqueName => "Injection by RtlCreateUserThread API";

        public string About => "API: RtlCreateUserThread" + Environment.NewLine +
        ...
        private delegate int RtlCreateUserThread(
